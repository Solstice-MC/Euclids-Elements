...
public abstract class EnchantmentHelperMixin {
	...
	@Shadow public static void forEachEnchantment(ItemStack stack, EnchantmentHelper.Consumer consumer) {}

	@Shadow public static void forEachEnchantment(LivingEntity entity, EnchantmentHelper.ContextAwareConsumer contextAwareConsumer) {}

	@Shadow public static void forEachEnchantment(ItemStack stack, EquipmentSlot slot, LivingEntity entity, EnchantmentHelper.ContextAwareConsumer contextAwareConsumer) {}
...
//        );
	...
	private static void onProjectileSpawned(ServerWorld world, ItemStack stack, PersistentProjectileEntity projectileEntity, Consumer<Item> onBreak, Operation<Void> original) {
		...
		forEachEnchantment(stack, slot, entity, (effectHolder, level, context) ->
	...
	private static void onHitBlock(ServerWorld world, ItemStack stack, LivingEntity user, Entity enchantedEntity, EquipmentSlot slot, Vec3d pos, BlockState state, Consumer<Item> onBreak, Operation<Void> original) {
		forEachEnchantment(stack, slot, user, (effectHolder, level, context) ->
	...
	private static float getCrossbowChargeTime(ItemStack stack, LivingEntity user, float base, Operation<Float> original) {
		MutableFloat result = new MutableFloat(base);
		forEachEnchantment(user, (effectHolder, level, context) ->
