...
public class MapTagLoader<T, R> implements SimpleSynchronousResourceReloadListener {
	...
	public void reload(ResourceManager manager) {
		...
		Map<RegistryKey<Registry<T>>, Map<MapTagKey<T, R>, List<MapTagData<R>>>> contents = new HashMap<>();
		this.registryAccess.streamAllRegistryKeys()
			.map(key -> (RegistryKey<Registry<T>>) key)
			.forEach(key -> contents.put(key, getAllContents(manager, ops, key)));

		contents.forEach((registryReference, fileData) -> {
			Registry<T> registry = this.registryAccess.get(registryReference);
			fileData.forEach((mapTagKey, content) -> {
	...
	public static <T, R> Map<MapTagKey<T, R>, List<MapTagData<R>>> getAllContents (
		ResourceManager manager,
		RegistryOps<JsonElement> ops,
		RegistryKey<Registry<T>> registryKey
	) {
		Map<MapTagKey<T, R>, List<MapTagData<R>>> results = new HashMap<>();
		...
		finder.findAllResources(manager).forEach((path, resources) -> {
			...
			List<MapTagData<R>> contents = readContents(ops, mapTagKey, resources);
	...
	@SuppressWarnings("unchecked")
	public static <R> List<MapTagData<R>> readContents (
		RegistryOps<JsonElement> ops,
		MapTagKey<?, ?> mapTagKey,
		List<Resource> resources
	) {
		List<MapTagData<R>> result = new LinkedList<>();
		resources.forEach(resource -> {
			try (Reader reader = resource.getReader()) {
				JsonElement element = JsonParser.parseReader(reader);
				Codec<MapTagData<R>> codec = MapTagData.codec((MapTagKey<?, R>) mapTagKey);
